...
function lerSaidaPendente(): boolean {
  try {
    return sessionStorage.getItem(CHAVE_SAIDA_PENDENTE) === '1';
  } catch {
    return false;
  }
}
...
function gravarSaidaPendente(pendente: boolean): void {
  try {
    if (pendente) {
      sessionStorage.setItem(CHAVE_SAIDA_PENDENTE, '1');
    } else {
      sessionStorage.removeItem(CHAVE_SAIDA_PENDENTE);
    }
  } catch {
    // Armazenamento indisponível (navegação privada, por exemplo). A sessão
    // segue funcionando; apenas a proteção extra do logout não se aplica.
  }
}
...
export function ProvedorDeSessao({ cliente, children }: PropsDoProvedor) {
  ...
  const entrar = useCallback(
    async (email: string, senha: string) => {
      ...
      // Entrar de novo cancela qualquer saída pendente: a sessão anterior
      // deixou de importar e esta é uma decisão explícita do usuário.
