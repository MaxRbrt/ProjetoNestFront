...
async function extrairErro(resposta: Response): Promise<ApiError> {
  ...
  try {
    ...
    // Resposta sem corpo JSON: a mensagem padrão já cobre o caso.
  }
...
export class ApiClient {
  ...
  // tentar de novo, mesmo que a sessão anterior tenha terminado mal.
  // ---------------------------------------------
  async recuperarSessao(): Promise<SessaoAutenticada> {
    ...
    if (this.geracao !== geracaoInicial) {
      // O usuário entrou ou saiu enquanto a resposta vinha: aplicar agora
      // trocaria a sessão atual por uma que já não vale.
      return sessao;
    }
  ...
  private renovarSessao(): Promise<string> {
    ...
    this.renovacaoEmAndamento = this.comLock(async () => {
      ...
      if (this.geracao !== geracaoInicial) {
        return this.accessToken ?? sessao.accessToken;
      }
  ...
  private abrirCanal(nome: string): BroadcastChannel | null {
    ...
    canal.onmessage = (mensagem: MessageEvent<EventoDeSessao>) => {
      // Sem propagar de volta: a aba que originou o evento já aplicou o
      // estado, e reenviar criaria um eco entre as abas.
