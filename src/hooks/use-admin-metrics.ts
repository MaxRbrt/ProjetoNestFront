...
import type { Paginado } from '../api/products';
...
// ---------------------------------------------
export function useAdminMetrics(cliente: ApiClient): ResultadoDeMetricas {
  ...
  useEffect(() => {
    ...
    Promise.all([
      cliente.get<Paginado<unknown>>('/products?limit=1', {
        signal: controlador.signal,
      }),
      cliente.get<Paginado<unknown>>('/categories?limit=1', {
        signal: controlador.signal,
      }),
      cliente.get<Paginado<unknown>>('/orders?limit=1', {
        signal: controlador.signal,
      }),
    ])
